...
with open('Addresses.csv') as csv_file_name:
    readCSV_name = csv.reader(csv_file_name, delimiter=',')
    readCSV_name = list(readCSV_name)
...
#This fuction generates the total distance for the second  truck
    def check_second_truck_time(distances):
        time_new = distances / 18
        distance_in_minutes = '{0:02.0f}:{1:02.0f}'.format(*divmod(time_new * 60, 60))
        time_final = distance_in_minutes + ':00'
        second_truck_time.append(time_final)
        sum = datetime.timedelta()
...
#This fuction generates the total distance for the third truck
    def check_third_truck_time(distances):
        time_new = distances / 18
        distance_in_minutes = '{0:02.0f}:{1:02.0f}'.format(*divmod(time_new * 60, 60))
        time_final = distance_in_minutes + ':00'
        third_truck_time.append(time_final)
        sum = datetime.timedelta()
        for i in third_truck_time:
            (h, m, s) = i.split(':')
            d = datetime.timedelta(hours=int(h), minutes=int(m), seconds=int(s))
            sum += d
        return sum
            ...
            try:
                ...
                for index in truck_distance_list:
                    if check_current_distances(current_location, int(index[1])) <= lowest_value:
                        lowest_value = check_current_distances(current_location, int(index[1]))
